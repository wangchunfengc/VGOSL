...
class build_transformer_local(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory, rearrange):
        ...
        self.part_gate = nn.Sequential(
            ...
            nn.Linear(self.in_planes, self.in_planes // 4),
            nn.ReLU(inplace=True),
            nn.Linear(self.in_planes // 4, 1),
            nn.Sigmoid()
        )
        self.alpha_vis = nn.Parameter(torch.tensor(0.5))

        self.bottleneck_cat = nn.BatchNorm1d(self.in_planes * 5)
        self.bottleneck_cat.bias.requires_grad_(False)
        self.bottleneck_cat.apply(weights_init_kaiming)
    def patchmask_to_partvis_by_token(self, mask_patch, patch_length):
        B, gh, gw = mask_patch.shape
        m = mask_patch.reshape(B, gh * gw)
        ...
        for k in range(4):
            ...
            b = min(b, gh * gw)
            a = min(a, gh * gw - 1)
            v_k = m[:, a:b].mean(dim=1)  # [B]
            v.append(v_k)
        return torch.stack(v, dim=1)  # [B,4]

    def vis_loss(self, w_logit, v_gt):
        # w_logit: [B,4] logits
        # v_gt:    [B,4] float in [0,1]
        return F.binary_cross_entropy_with_logits(w_logit, v_gt)

    def forward(self, x, x2, x3, mask_patch=None,label=None, cam_label= None, view_label=None):  # label is unused if self.cos_layer == 'no'
        ...
        local_raw_1, local_raw_2, local_raw_3, local_raw_4 = local_feat_1, local_feat_2, local_feat_3, local_feat_4

        parts_raw = torch.stack([local_raw_1, local_raw_2, local_raw_3, local_raw_4], dim=1)  # [B,4,768]
        w = torch.stack([
            self.part_gate(local_raw_1),
            self.part_gate(local_raw_2),
            self.part_gate(local_raw_3),
            self.part_gate(local_raw_4),
        ], dim=1)  # [B,4,1]
        w_norm = w / (w.sum(dim=1, keepdim=True) + 1e-6)
        global_from_parts = (w_norm * parts_raw).sum(dim=1)  # [B,768]

        global_feat = global_feat_ori + self.alpha_vis * global_from_parts

        L_vis = None
        if self.training and (mask_patch is not None):
            v_gt = self.patchmask_to_partvis_by_token(mask_patch, patch_length)  # [B,4]
            L_vis = self.vis_loss(w.squeeze(-1), v_gt)  # [B,4] vs [B,4]
        ...
        if self.training:
            ...
            return [cls_score, cls_score_occ1, cls_score_occ2, cls_score_1, cls_score_2, cls_score_3,
                        cls_score_4
                            ...
                            local_feat_4]  ,{"L_vis": L_vis}# global feature for triplet loss
        else:
            return torch.cat(
